...
\paragraph{13} Every thing that generates from its own es\-sence generates
...
simple es\-sence, and because the reason of the soul is self-moved.

\paragraph{23} The intellectual es\-sence is homogeneous, so that the things
...
\paragraph{24} The death of the es\-sence whose being is in life, and whose
...
\paragraph{30} Of es\-sences which are whole and perfect no one turns towards
its own offspring. But all the perfect es\-sences are carried up to the things
which produced them, from the cosmic body upwards. For it, being perfect, is
carried up toward the soul, which is intellectual. And for this reason it moves
in a circle. And the soul of it [is carried up] toward intellect, and intellect
...
a shade, whose nature it is to be in space, but which possesses its es\-sence
in darkness. So that if Hades is a dark subterranean place, the soul, though
not abstracted from being, comes into Hades, drawing the shade after it. For
when it has gone out from the solid body, the spirit which it has collected to
it from the spheres follows it. But as, from its sympathy with the body, it has
...
ground: not because the same es\-sence traverses places, and comes into places,
...
changeable, that it has its es\-sence in otherness, that it is composite, that
it has [no] existence in and for itself, that it is intuited in place and in
mass, and so forth. On the other hand, [the things that can be affirmed] of
that which essentially is, are, that it exists in and for itself; that it is
always situated within itself, and similarly that it always is in the same
manner; that its es\-sence is invested with identity; that it is unchangeable
in its es\-sence; that it is incomposite, indissoluble, and not in place, or
diffused into mass; that it neither becomes nor decays, and so forth. Adhering
to these [distinctions], we ought not, in speaking, to make any confusion
between their different natures, or to listen to others when they in speaking
do so.
...
of es\-sence. For as mass was seen to be a going outside of itself, and a
...
things thus differing in es\-sence. So there is great distance between the
world and the power of being, and between being and the impotence of the
material. But that which lies between, assimilating and assimilated, uniting
the extremities, has been the cause of error in regard to the extremities, by
...
\paragraph{42} That which has its being in another, and which has no es\-sence
in itself apart from another, if it turns to itself in order to cognize itself
apart from that in which it has its es\-sence, separating itself from that, is
...
of itself, cannot possibly have its es\-sence in that from which it is able to
